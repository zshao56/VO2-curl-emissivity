...
-- 循环遍历每个文件
for _, file_name in pairs({'Al2O3.csv', 'Ge.csv', 'GST.csv', 'Ni.csv', 'ZnS.csv'}) do
...
for wvl = 4, 140, 1 do
    ...
    S:SetNumG(1)
    freq = 10 / wvl
    S:SetMaterial('Al2O3', {r_arrays['Al2O3.csv'][wvl], i_arrays['Al2O3.csv'][wvl]})
    S:SetMaterial('Ge', {r_arrays['Ge.csv'][wvl], i_arrays['Ge.csv'][wvl]})
    S:SetMaterial('ZnS', {r_arrays['ZnS.csv'][wvl], i_arrays['ZnS.csv'][wvl]})
    S:SetMaterial('GST', {r_arrays['GST.csv'][wvl], i_arrays['GST.csv'][wvl]})
    S:SetMaterial('Ni', {r_arrays['Ni.csv'][wvl], i_arrays['Ni.csv'][wvl]})
    S:SetMaterial('Vacuum', {1,0})

    S:AddLayer('AirAbove', 0, 'Vacuum')
    S:AddLayer('slab1', 0.065, 'Al2O3')

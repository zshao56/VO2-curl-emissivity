...
output_file_path = string.format('%s\\lt-VO2curl1.txt', output_folder)
...
halfwidth_values = {25, 50, 75, 100, 125, 150}

local output_file = io.open(output_file_path, 'a')
output_file:write("halfwidth*Y2O3thickness")
for _, halfwidth in ipairs(halfwidth_values) do
    for _, thickness in ipairs(thickness_values) do
        output_file:write(string.format("\t%s*%.1f", halfwidth, thickness))
    end
end
...
for wvl = 4, 140, 1 do
    output_file:write(string.format("%f", wvl/10))
    for _, halfwidth in ipairs(halfwidth_values) do
        for _, thickness in ipairs(thickness_values) do

            S = S4.NewSimulation()
            S:SetLattice({300,0}, {0,300})
            S:SetNumG(1)
            freq = 10 / wvl
            ...
            S:AddLayer('slab1', 0.025, 'VO2_25_CCC')
            S:AddLayer('slab2', thickness, 'Y2O3_CCC')
            S:AddLayer('slab3', 0.32, 'ITO_CCC')
            S:AddLayerCopy('AirBelow', 0, 'AirAbove')

            S:SetLayerPatternRectangle('slab1', 'Vacuum', {0,0}, 0, {halfwidth,halfwidth})
            ...
            output_file:write(string.format("\t%f", Absorption))
